...
; test-compiler compares the output of eval-llvm with eval-top-level.
(define ((make-test path) exp ext)
  (lambda ()
            (test-compiler (top-level exp)
                                      (proc->llvm (closure-convert (cps-convert (anf-convert (alphatize (assignment-convert (simplify-ir (desugar (top-level exp)))))))))))
            )
